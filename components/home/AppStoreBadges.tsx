...
// sin URL caen en "#" para no romper el layout.
...
export function AppStoreBadges({ variant = "dark", className = "", storeUrl, playUrl }: Props) {
  ...
  // target solo en links reales: "#" en nueva pestaña sería molesto
  const ext = (url?: string) =>
    url ? { target: "_blank", rel: "noopener noreferrer" } : {};

  return (
    <div className={`flex flex-wrap items-center gap-3 ${className}`}>
      <a href={storeUrl ?? "#"} {...ext(storeUrl)} className={`flex items-center gap-3 ${base} rounded-2xl pl-4 pr-5 py-3 card-hover`}>
      ...
      <a href={playUrl ?? "#"} {...ext(playUrl)} className={`flex items-center gap-3 ${base} rounded-2xl pl-4 pr-5 py-3 card-hover`}>
